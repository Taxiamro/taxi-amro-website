...
export default function PriceCalculator() {
  ...
  const handleWhatsApp = () => {
    if (!result) return
    const fp = retour ? Math.round(result.price * 0.9) : result.price
    let m = `Hallo, ik wil graag een rit boeken:\nVan: ${result.origin}\nNaar: ${result.dest}\nGeschatte prijs: €${fp}`
    if (retour) m += `\nRetour: ja${retourTijd ? ` (retour ophaaltijd ${retourTijd.replace('T', ' ')})` : ''}`
    if (zakelijk) m += `\nZakelijk: graag een factuur`
    const msg = encodeURIComponent(m)
    window.open(`https://wa.me/31633721505?text=${msg}`, '_blank')
  }

  const finalPrice = result ? (retour ? Math.round(result.price * 0.9) : result.price) : 0
                ...
                <div className="relative bg-gray-900 rounded-2xl p-6 overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="text-gray-400 text-xs uppercase tracking-wide mb-1">Geschatte ritprijs</div>
                      ...
                      {(result.discount.pct > 0 || retour) && (
                        <span className="text-gray-500 line-through text-xl mb-1">€{result.rawPrice}</span>
                      )}
                      <div className="text-yellow-400 font-bold text-4xl">€ {finalPrice}</div>
                    </div>
                    ...
                    {retour && (
                      <div className="inline-flex items-center gap-1 bg-green-500/20 text-green-400 text-xs font-semibold px-3 py-1 rounded-full mb-1 ml-1">✓ Extra 10% retour korting</div>
                    )}
                    <div className="text-gray-400 text-sm mb-1">Afstand: {result.km.replace('.', ',')} km</div>
                    <div className="text-gray-600 text-xs mb-4">Vaste prijs op basis van officieel metertarief.</div>

                    <div className="text-gray-400 text-[11px] uppercase tracking-wide mb-2">Kortingen toepassen</div>
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      <button type="button" onClick={() => setRetour(v => !v)}
                        className={`flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl text-xs font-semibold border transition-all ${retour ? 'bg-green-500/15 border-green-500 text-green-400' : 'bg-gray-800 border-gray-700 text-gray-300 hover:border-gray-500'}`}>
                        🔄 Retour rit 10%
                      </button>
                      <button type="button" onClick={() => setZakelijk(v => !v)}
                        className={`flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl text-xs font-semibold border transition-all ${zakelijk ? 'bg-blue-500/15 border-blue-400 text-blue-300' : 'bg-gray-800 border-gray-700 text-gray-300 hover:border-gray-500'}`}>
                        💼 Zakelijk / factuur
                      </button>
                    </div>
                    {retour && (
                      <div className="mb-4">
                        <label className="text-gray-400 text-[11px] uppercase tracking-wide block mb-1">Retour ophaaltijd</label>
                        <input type="datetime-local" value={retourTijd} onChange={(e) => setRetourTijd(e.target.value)}
                          className="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-amber-400" />
                      </div>
                    )}
                    <button
                    ...
                    >
                      ...
                      Boek voor €{finalPrice} via WhatsApp
                    </button>
                  </div>
                </div>
              </div>
